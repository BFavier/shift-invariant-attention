...
device = "cuda:0"
train_data = Batchifyer(x_train, y_train, classes, device=device, n_batches=1)
val_data = Batchifyer(x_val, y_val, classes, device=device, n_batches=1)
model = ShiftInvariantTransformer(classes, n_stages=4, projection_dim=8, n_heads=4, t_scaling_factors=[1.])
model.to(device)
